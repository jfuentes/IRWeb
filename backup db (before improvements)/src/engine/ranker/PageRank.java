...
public class PageRank {
		...
		for(int iteration=0; iteration<NUMBER_ITERATIONS; iteration++){
			...
			for(Link link = links.first(); link!=null; link=links.next()){
				...
				for(int i=0; i<ingoingLinks.size(); i++){
					...
					if(l!=null){
						totalIngoingLinks+= l.getCurrentPageRank()/(l.getNumberOutgoingLinks()==0?1:l.getNumberOutgoingLinks());
	...
	public static void printHighestPageRanks(int top) {
		...
		System.out.println("Pagerank websites highest Top "+top);
		for(int i=arrayList.size()-1; i>=arrayList.size()-top; i--){
			System.out.println((i+1)+") "+arrayList.get(i).getURL()+":  "+arrayList.get(i).getCurrentPageRank()+"    ingoing links: "+arrayList.get(i).getIngoingLinks().size());
		}
		System.out.println();
		System.out.println("Pagerank websites lowest Top "+top);
		for(int i=0; i<top; i++){
			System.out.println((i+1)+") "+arrayList.get(i).getURL()+":  "+arrayList.get(i).getCurrentPageRank()+"    ingoing links: "+arrayList.get(i).getIngoingLinks().size());
		}
	}
	...
	public static void removeNonICS() {
		// TODO Auto-generated method stub
		LinksDB linksDB = LinksDB.getInstance();
		EntityCursor<Link> links = linksDB.getCursorLinks();
		System.out.println("Number of websites before removing "+linksDB.getTotalLinks());
		for(Link link = links.first(); link!=null; link=links.next()){
			if(!link.getURL().matches("^http://.*\\.ics\\.uci\\.edu/.*")){
				linksDB.deleteWebpage(link.getURL());
			}
		}
		
		System.out.println("Number of websites after removing "+linksDB.getTotalLinks());
	}

	public static double findPageRankURL(String url) {
		// TODO Auto-generated method stub
		LinksDB linksDB = LinksDB.getInstance();
		Link link = linksDB.getLink(url);
		if(link!=null)
		return link.getCurrentPageRank();
		else 
			return -1;
	}
